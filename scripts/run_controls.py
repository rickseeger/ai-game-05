...
p.add_argument("output", help="new directory relative to repo/evidence/controls")
p.add_argument("--size", default="1280x720")
p.add_argument("--negative-collision", action="store_true")
a = p.parse_args()
...
out = ROOT / "evidence" / "controls" / a.output
...
cmd = ["xvfb-run", "-a", "-s", f"-screen 0 {max(width,1280)}x{max(height,1024)}x24", str(godot),
       "--path", str(ROOT / "game"), "res://controls.tscn", "--rendering-method", "gl_compatibility",
       "--audio-driver", "Dummy", "--resolution", a.size, "--disable-vsync", "--max-fps", "60"]
cmd += ["--", "--controls-test"]
if a.negative_collision:
    cmd += ["--negative-collision"]
env = dict(os.environ, CONTROLS_OUT=str(out), GODOT_SILENCE_ROOT_WARNING="1", LIBGL_ALWAYS_SOFTWARE="1")
...
metadata = {"command": cmd, "environment": {k: env[k] for k in
    ("CONTROLS_OUT", "GODOT_SILENCE_ROOT_WARNING", "LIBGL_ALWAYS_SOFTWARE")},
