...
p=argparse.ArgumentParser(); p.add_argument('output'); p.add_argument('--size',default='1280x720'); a=p.parse_args()
...
cmd=['xvfb-run','-a','-s',f'-screen 0 {max(w,1280)}x{max(h,1024)}x24',godot,'--path',str(ROOT/'game'),'--script','res://tests/dense_visibility_tests.gd','--rendering-method','gl_compatibility','--audio-driver','Dummy','--resolution',a.size,'--disable-vsync','--max-fps','60']
